...
func (c *Client) readPump(world *game.World) {
	...
	for {
		...
		if err = json.Unmarshal(message, &event); err != nil {
			log.Println("error unmarshal in readPump():", err)
		}
...
func serveWs(hub *Hub, world *game.World, w http.ResponseWriter, r *http.Request) {
	...
	if err = conn.WriteJSON(game.Event{
		...
		log.Println("error conn.WriteJSON():", err)
	}

	m, err := json.Marshal(game.Event{
	...
	if err != nil {
		log.Println("error marshal in serveWs():", err)
	}
	hub.broadcast <- m
